...
(defonce all
  [[:a "A" nil :core]
   ...
   ;; i11n
   [:iso# "ISO#" ["HASH" "NUMBER" "#"] :i11l]
   [:iso-slash "ISO/" ["ISO/" "ISO SLASH"] :i11l]
   [:intl1 "INTL1" ["INTER1" "KANJI1"] :i11l]
   [:intl2 "カナ" ["INTER2" "KANJI2" "KANA" "カナ"] :i11l]
   [:intl3 "¥" ["INTER3" "KANJI3" "YEN" "¥"] :i11l]
   [:intl4 "変換" ["INTER4" "KANJI4" "HENKAN" "変換"] :i11l]
   [:intl5 "無変換" ["INTER5" "KANJI5" "MUHENKAN" "無変換"] :i11l]
   [:intl6 "INTL6" ["INTER6" "KANJI6"] :i11l]
   [:intl7 "INTL7" ["INTER7" "KANJI7" "BYTETOGGLE"] :i11l]
   [:intl8 "INTL8" ["INTER8" "KANJI8"] :i11l]
   [:intl9 "INTL9" ["INTER9" "KANJI9"] :i11l]
   [:lang1 "한/영" ["LANG1" "HANGULENGLISH" "HANGUL ENGLISH" "한/영"] :i11l]
   [:lang2 "英数/한자" ["LANG2" "HANJA" "EISU" "英数/한자"] :i11l]
   [:lang3 "カタカナ" ["LANG3" "KATAKANA" "カタカナ"] :i11l]
   [:lang4 "ひらがな" ["LANG4" "HIRAGANA" "ひらがな"] :i11l]
   [:lang5 "半角/全角" ["LANG5" "ZENKAKUHANKAKU" "ZENKAKU HANKAKU" "半角/全角"] :i11l]
   [:lang6 "LANG6" ["LANG6"] :i11l]
   [:lang7 "LANG7" ["LANG7"] :i11l]
   [:lang8 "LANG8" ["LANG8"] :i11l]
   [:lang9 "LANG9" ["LANG9"] :i11l]

   ;; mouse
   [:m-btn1 "Btn 1" ["#:mouseOut( 1, 0, 0 )"] :mouse]
   [:m-btn2 "Btn 2" ["#:mouseOut( 2, 0, 0 )"] :mouse]
   [:m-btn3 "Btn 3" ["#:mouseOut( 3, 0, 0 )"] :mouse]
   [:m-btn4 "Btn 4" ["#:mouseOut( 4, 0, 0 )"] :mouse]
   [:m-btn5 "Btn 5" ["#:mouseOut( 5, 0, 0 )"] :mouse]
   [:m-btn6 "Btn 6" ["#:mouseOut( 6, 0, 0 )"] :mouse]
   [:m-btn7 "Btn 7" ["#:mouseOut( 7, 0, 0 )"] :mouse]
   [:m-btn8 "Btn 8" ["#:mouseOut( 8, 0, 0 )"] :mouse]
   [:m-btn9 "Btn 9" ["#:mouseOut( 9, 0, 0 )"] :mouse]
   [:m-btn10 "Btn 10" ["#:mouseOut( 10, 0, 0 )"] :mouse]
   [:m-btn11 "Btn 11" ["#:mouseOut( 11, 0, 0 )"] :mouse]
   [:m-btn12 "Btn 12" ["#:mouseOut( 12, 0, 0 )"] :mouse]
   [:m-btn13 "Btn 13" ["#:mouseOut( 13, 0, 0 )"] :mouse]
   [:m-btn14 "Btn 14" ["#:mouseOut( 14, 0, 0 )"] :mouse]
   [:m-btn15 "Btn 15" ["#:mouseOut( 15, 0, 0 )"] :mouse]
   [:m-btn16 "Btn 16" ["#:mouseOut( 16, 0, 0 )"] :mouse]
   [:m-up1 "↑ x1" ["#:mouseOut( 0, 0, 1 )"] :mouse]
   [:m-up5 "↑ x5" ["#:mouseOut( 0, 0, 5 )"] :mouse]
   [:m-dn1 "↓ x1" ["#:mouseOut( 0, 0, -1 )"] :mouse]
   [:m-dn5 "↓ x5" ["#:mouseOut( 0, 0, -5 )"] :mouse]
   [:m-lf1 "← x1" ["#:mouseOut( 0, -1, 0 )"] :mouse]
   [:m-lf5 "← x1" ["#:mouseOut( 0, -5, 0 )"] :mouse]
   [:m-rt1 "→ x1" ["#:mouseOut( 0, 1, 0 )"] :mouse]
   [:m-rt5 "→ x1" ["#:mouseOut( 0, 5, 0 )"] :mouse]
   ...
   ;; multimedia
   [:mute "MUTE" ["CONS:MUTE"] :mmed]
   [:volup "VOL+" ["CONS:VOLUMEUP" "CONS:VOLUME UP"] :mmed]
   [:voldn "VOL-" ["CONS:VOLUMEDOWN" "CONS:VOLUME DOWN"] :mmed]
   [:plpa "PL/PS" ["CONS:PAUSEPLAY"] :mmed]
   [:play "PLAY" ["CONS:PLAY"] :mmed]
   [:pause "PAUSE" ["CONS:PAUSE"] :mmed]
   [:ff "FF" ["CONS:FASTFORWARD"] :mmed]
   [:rew "RW" ["CONS:REWIND"] :mmed]
   [:next "NEXT" ["CONS:SCANNEXTTRACK"] :mmed]
   [:prev "PREV" ["CONS:SCANPREVIOUSTRACK"] :mmed]
   [:stop "STOP" ["CONS:STOP"] :mmed]
   [:ejec "EJECT" ["CONS:EJECT"] :mmed]
   [:calc "CALC" ["CONS:CALCULATOR"] :mmed]
   [:web "WEB" ["CONS:INTERNETBROWSER"] :mmed]
   [:files "FILES" ["CONS:FILEBROWSER"] :mmed]
   [:scrup "SCRN+" ["CONS:BRIGHTNESSINCREMENT"] :mmed]
   [:scrdn "SCRN-" ["CONS:BRIGHTNESSDECREMENT"] :mmed]
   [:power "POWER" ["SYS:POWERDOWN"] :mmed]
   [:sleep "SLEEP" ["SYS:SLEEP"] :mmed]
   [:wake "WAKEUP" ["SYS:WAKEUP"] :mmed]
