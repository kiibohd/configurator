...
import { PredefinedKey } from './types';
...
const keymap: Dictionary<PredefinedKey> = _.keyBy(keys, 'name');
const categorized: Dictionary<PredefinedKey[]> = _.chain(keys)
  .filter('group')
  .groupBy('group')
  .value();

const aliases: Dictionary<PredefinedKey> = keys.reduce((o, k) => {
  _.forEach(k.aliases, a => (o[a.replace(/\s/g, '').toUpperCase()] = k));
  return o;
}, {} as Dictionary<PredefinedKey>);
